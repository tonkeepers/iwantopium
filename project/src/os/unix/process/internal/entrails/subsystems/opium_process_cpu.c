...
#pragma region global_segment 
   ...
   void
opium_process_cpu_set_affinity(opium_u64_t *mask, size_t n)
{
   ...
   void
opium_process_cpu_set_policy(opium_s32_t policy)
{
   opium_process_cpu_config_t *config = getconfig(base);
   ...
   void
opium_process_cpu_set_nice(opium_s32_t nice)
{
   opium_process_cpu_config_t *config = getconfig(base);
   config->nice = nice;
}
...
static opium_s32_t config_validate() 
{
   ...
   opium_u32_t cpu_count = sysconf(_SC_NPROCESSORS_ONLN);

   if (cpu_count <= 0) {
      opium_log_err(base->log, "Failed to determine number of CPUs.\n");
      return OPIUM_RET_ERR;
   }
   ...
   if (cpu_count > 0) {
      ...
      for (size_t index = 0; index < cpu_count && index < CPU_SETSIZE; index++) {
      ...
      for (size_t index = 0; index < CPU_SETSIZE; index++) {
         if (index >= cpu_count) {
            opium_log_err(base->log, "Invalid CPU %d (system has only %d CPU(s))", index, cpu_count);
            return OPIUM_RET_ERR;
         }
      }
   } 
