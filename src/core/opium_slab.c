...
opium_slab_init(opium_slab_t *slab, size_t item_size, opium_log_t *log)
{
   ...
   opium_log_debug(log, 
         ...
         slab->item_size, opium_slab_count(slab), data_offset, slab->page_size);
...
opium_slab_alloc(opium_slab_t *slab)
{
   ...
   if (opium_likely(!opium_list_empty(&slab->partial))) {
      ...
      page->mask = opium_slab_page_init(opium_slab_count(slab));
      ...
      if (slab->page_size <= OPIUM_SLAB_PAGE_SIZE) {
         ...
         page = mem_ops->alloc_page(slab->pages_per_alloc, slab->log);
         ...
         page = mem_ops->alloc_aligned(slab->page_size,
               slab->pages_per_alloc, slab->log);
         if (opium_unlikely(!page)) {
            slab->stats.fails = slab->stats.fails + 1;
      ...
      opium_slab_slots_for_each(current, pos, end, slab->page_size) {
         ...
         current->boss = page;
         current->mask = opium_slab_page_init(opium_slab_count(slab));
         current->refcount = 0;
...
opium_slab_free(opium_slab_t *slab, void *ptr)
{
   ...
   if (opium_unlikely(page->mask == OPIUM_SLAB_PAGE_BUSY)) {
      ...
      if (opium_unlikely(boss->refcount == 1)) {
         ...
         if (slab->page_size <= OPIUM_SLAB_PAGE_SIZE) {
            ...
            mem_ops->free_page(boss, slab->pages_per_alloc, slab->log);
         } else {
            // opium_free(boss, slab->log);
            mem_ops->free_aligned(boss, slab->log);
         }
   ...
   opium_u32_t 
opium_slab_count(opium_slab_t *slab)
{
   return slab->item_count;
}

   opium_u32_t
opium_slab_used(opium_slab_t *slab)
{
   return slab->stats.used;
}
...
opium_slab_traverse(opium_slab_t *slab, opium_slab_trav_ctx func)
{
   ...
   opium_list_for_each_entry_safe(page, tmp, &slab->full, head) {
      for (size_t index = 0; index < opium_slab_count(slab); index++) {
   ...
   for (size_t index = 0; index < 3; index++) {
      ...
      opium_list_for_each_entry_safe(current, tmp, current_head, head) {
         if (current && current->refcount != 0) {
            size_t used_mask = current->mask & ((1ULL << opium_slab_count(slab)) - 1);
            size_t used = __builtin_popcountll(used_mask);
            size_t free = opium_slab_count(slab) - used;
